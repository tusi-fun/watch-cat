...
@Order(2)
@Aspect
public class LimitCatAspect {

    @Autowired
    private LimitCatService limitCatService;

    @Pointcut("@annotation(limitCat)")
    public void pointCut(LimitCat limitCat) {}
     ...
     * @throws Throwable
     */
    @Around("pointCut(limitCat)")
    public Object doAround(ProceedingJoinPoint proceedingJoinPoint, LimitCat limitCat) throws Throwable {

        String scene = StringUtils.hasText(limitCat.scene())?limitCat.scene():proceedingJoinPoint.getSignature().getName();
        String key = getKey(proceedingJoinPoint,scene,limitCat.key());
     ...
     * @param e
     */
    @AfterThrowing(pointcut = "pointCut(limitCat)", throwing = "e")
    public void doAfterThrow(JoinPoint joinPoint, RuntimeException e, LimitCat limitCat) {
