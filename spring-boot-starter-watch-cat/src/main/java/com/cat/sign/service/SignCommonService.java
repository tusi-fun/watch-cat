...
public class SignCommonService {
    ...
    public SignCommonService(RedisTemplate wcRedisTemplate) {
    ...
    public Boolean checkSign(String sign, Duration timeout) {
        return wcRedisTemplate.opsForValue().setIfAbsent(String.format(SAFETY_API_SIGN_KEY, SecureUtil.md5(sign)), sign, timeout);
    }
